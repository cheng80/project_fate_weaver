...
이 프로젝트의 최종 목표는 YAML 검증 도구를 만드는 것이 아니라, 플레이어가 매 선택마다 위험과 보상을 판단하고, 그 결과가 다음 사건과 생존 가능성을 바꾸는 게임 경험을 만드는 것입니다.

Console Simulator와 YAML 검증은 최종 게임이 아니라, 그 경험이 실제로 작동하는지 빠르게 확인하기 위한 실험실입니다.

## 목적

Project FateWeaver는 **이벤트 중심 판타지 로그라이크**를 만들기 위한 프로젝트다.

현재 단계의 목표는 Flutter/Flame 앱 제작이 아니라, **Console Validation**으로 검증된 핵심 루프 위에서 콘텐츠 확장 준비 상태를 점검하는 것이다.
...
```text
Python 콘솔 기반 이벤트-선택-상태 변화 루프 유지
YAML 데이터 계약 검증
scenario 기반 테스트 범위 검증
로그/재미 지표 검증
Ontology-lite 관계 계약 유지
콘텐츠 확장 readiness 점검
```
...
```text
READY_FOR_CONTENT_EXPANSION_READINESS
```

다음 작업:

```text
Content Expansion Readiness 검수
콘텐츠 확장 계획 수립
