...
| press_forward | 빠르게 전진한다 | health/curse risk | event progress, money |
| retreat | 큰 피해를 피한다 | opportunity loss | health/curse 보존 |
| bargain | 사회적/경제적 비용을 낸다 | money/reputation | 접근권, clue |
| exploit | 위험한 이득을 노린다 | curse/health | money/item |
| cleanse | 오염이나 위험을 줄인다 | item/money | curse 감소 |
...
| investigate | 단서를 깊게 본다 | time/food/curse risk | mystery signal |
...
| curse pressure | curse 증가, future weight 악화 | cleanse, avoid, bargain |
...
좋은 reward/cost 조합은 profile별 선택 차이를 만든다. 예를 들어 안전 profile은 curse/health 비용을 싫어하고, greedy profile은 money/item 보상을 더 선호하며, desperate profile은 낮은 health/food 상태에서 회복 선택을 더 선호할 수 있어야 한다.
...
- 비용 있는 확인: 정보를 얻으려면 food, money, health, curse 중 하나를 지불한다.
...
| convert_cost | health/curse 비용을 item/money 비용으로 바꾼다 | consume_item, status_delta |
...
## 9. Curse/Mystery/Survival 패턴

### 9.1 Curse

- curse는 분위기 수치가 아니라 다음 사건과 선택을 왜곡하는 압력이다.
- curse 증가는 즉시 보상과 묶을 때 tradeoff가 선명하다.
- curse 감소는 비용이 있어야 하며, 항상 정답이어서는 안 된다.
