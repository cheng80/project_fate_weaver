...
5. `greedy_leaning`이 money, item, reputation 보상을 노릴 수 있도록 curse, health, reputation 비용이 붙은 유혹을 둔다.
...
| `hollow_witness_bargain` | bargain | 정보와 비용의 교환 | trade, curse | bargain, investigate, exploit | 별도 NPC 시스템 없이 사회적 거래를 추가한다. |
...
| `black_pollen_contamination` | contamination | 보상을 위한 curse 감수 | curse, magic | cleanse, gamble, item_use | greedy/desperate profile에 명확한 bad tradeoff 유혹을 제공한다. |
...
| `resonance_lens` | [information, risk_reduce] | [tool] | [curse, ancient, darkness] | magic/curse 이벤트의 clue 구조를 드러내고 해석 위험을 낮춘다. |
| `waybread_pouch` | [cost_convert, risk_reduce] | [consumable, travel] | [hunger, lost] | survival 압력을 consumable resource choice로 전환한다. |

아이템 제약:

- `signal_chalk`는 2개 choice를 열고 future weight를 조정하되 health나 curse를 직접 줄이지 않는다.
- `resonance_lens`는 2개 choice에서 단서를 드러내거나 명확히 해야 하며, curse pressure를 줄이는 choice는 최대 1개로 둔다.
...
| `safe_leaning` | bad tradeoff를 낮게 유지한다 | safe/retreat/observe choice가 health나 curse를 보존하되 기회비용을 가진다. |
| `greedy_leaning` | 보상 중심성을 유지한다 | 최소 3개 high reward choice가 money, item, reputation을 주고 curse, health, reputation 비용을 요구한다. |
