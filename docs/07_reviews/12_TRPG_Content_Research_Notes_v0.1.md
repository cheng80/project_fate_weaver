...
- `curse`는 단순 벌점이 아니라 앞으로의 사건 해석을 바꾸는 압력이어야 한다.
...
- 시간 압력: 탐험 이벤트는 food, health, curse, future weight로 지연 비용을 만든다.
...
- health 손실만 반복하지 않고 item 소모, curse 증가, reputation 변화, 향후 위험 감소를 섞는다.
...
| 패턴 | 비용 | 보상 | 좋은 사용처 |
|---|---|---|---|
| 안전한 손실 | food/money 소량 감소 | curse/health 보존 | safe choice |
| 위험한 획득 | curse/health 악화 | money/item/정보 획득 | greedy/risky choice |
| 도구로 비용 전환 | item 소모 | health/curse 피해 감소 | item-based choice |
