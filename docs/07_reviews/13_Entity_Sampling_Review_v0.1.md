...
이번 작업은 문서화 작업이다. `data/content`, `data/core`, `data/scenarios`, `src`, `tools`는 수정하지 않았다.
...
- 탐험, 매복, 거래, 휴식, 저주, 굶주림, 길 잃음 같은 일반 상황/위협 명사
...
| 특정 시나리오 사건명 | `거짓 구조 신호`, `되돌아오는 표식`, `저주받은 경계` |
...
```text
현재 단계에서는 새 relation을 추가하지 않는다.
Phase 3 실행 중 반복 관계가 드러나면 위 조건을 만족하는지 별도 검토한다.
```
...
| 단서 | 별도 entity 없음 | event/choice/result message와 event_weight로 표현 | design-only |
...
```text
새 core entity는 지금 필요하지 않다.
Phase 3는 기존 event, choice, result, item, status, tag relation을 더 촘촘하게 쓰는 방향이 맞다.
```
...
| 오염/저주 이벤트 | curse 감소/증가 tradeoff | cleanse, gamble, item_based | bad tradeoff를 자연스럽게 만든다. |
...
| 저주 압력 | `curse`, `ancient`, `forbidden` | avoid, cleanse, risky reward | greedy와 safe 차이를 만든다. |
...
| 거짓 정보 | `lost`, `bandit`, `curse` 조합 | investigate, item reveal | 단서 기반 선택을 강화한다. |

보류 후보:

| 후보 | 보류 이유 |
|---|---|
| 정신력/광기 전용 위협 | 새 status를 유혹하므로 지금은 `curse`로 표현한다. |
...
| 공명 렌즈 | information, risk_reduce | curse, ancient, darkness | magic/curse 단서 해석을 돕되 보편 안전 해답이 되지 않는다. |
...
| 저주 완전 해제 부적 | `curse` 선택을 단순 정답으로 만들 위험이 크다. |
...
| `curse` | forbidden/magic/ancient 위험과 bad tradeoff를 표현한다. |

보류 후보:

| 후보 | 보류 이유 | 현재 대체 표현 |
|---|---|---|
| fear | 새 심리 수치보다 `curse`와 event_weight로 충분하다. | `curse`, `event_weight: {curse: +1}` |
...
| 검은 수액 차이 | risky investigate | reputation reward와 curse cost 연결 |
| 벌레 빛 배열 | investigate/gamble | magic/lost tradeoff |
| 거울 반사 이상 | item_based reveal | darkness/curse 해석 보조 |
...
| 저주 감소 | `status: {curse: -1}` | cleanse/item choice |
...
| 저주 증가 | `status: {curse: +1}` | gamble/magic/forbidden choice |
...
| 숲의 비인격적 반응 | magic/curse/lost tag | NPC 없이도 mystery 압력 제공 |
...
| 저주와 해석 실패 | curse, ancient, forbidden, magic clue | `event_has_danger_tag`, `result_changes_event_weight` |
...
```text
ENTITY_SAMPLING_READY
```

판정 이유:

- 현재 FateWeaver는 새 core entity를 추가하지 않아도 Phase 3 콘텐츠 표본을 만들 수 있다.
- 즉시 YAML화 후보는 기존 event/choice/result/item/status/tag 구조 안에서 표현 가능하다.
