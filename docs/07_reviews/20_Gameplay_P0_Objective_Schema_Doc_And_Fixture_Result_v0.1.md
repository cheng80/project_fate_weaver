...
`src/fateweaver/gameplay_p0.py`는 scenario의 `target_turns`를 P0 run budget으로 사용하도록 보강했다. `max_days`는 생존 강제 종료 조건이 아니라 Quest Report의 deadline failure reason으로 유지한다.

`src/fateweaver/gameplay_p0_objectives.py`는 `max_day_exceeded`가 result type을 failure로 만들도록 판정 경로를 보강했다. 기존 partial fixture에서 사용하는 `max_turn_exceeded`는 `return_late` reason과 함께 partial_success 흐름을 유지한다.
