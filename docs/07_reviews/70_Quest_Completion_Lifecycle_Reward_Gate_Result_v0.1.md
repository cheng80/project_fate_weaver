...
- `tests/test_gameplay_run_standard_run.py`
...
`quest_completed(...)`는 이미 objective satisfaction 기준으로 success를 판정하고 있었다. 문제는 `src/fateweaver/gameplay_run.py`와 `tools/manual_choice_runner.py`가 success break 조건에 `state.clock.turn >= _minimum_completion_turn(...)`을 hard gate로 붙인 점이었다.
...
`src/fateweaver/quest_lifecycle.py`를 추가했다.
