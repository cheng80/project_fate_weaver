...
def load_foundation(root: Path, quest_id: str | None) -> Foundation:
    ...
    quests = load_quest_mapping(root)
    cards = load_card_rule_mapping(root)
    score_rules = read_mapping(root / "data/core/score_rules.yaml")
    return Foundation(
        quest=_load_quest(quests, quest_id),
        card_rules=_load_card_rules(cards),
        score_rules=mapping_at(score_rules, "score_rules"),
    )
...
def _load_quest(raw: JsonMap, quest_id: str) -> Quest:
    for item in list_at(raw, "quests"):
        quest = as_mapping(item)
...
def _load_objectives(quest: JsonMap) -> tuple[QuestObjective, ...]:
    primary = tuple(_load_objective(item, True) for item in list_at(quest, "primary_objectives"))
    optional = tuple(_load_objective(item, False) for item in list_at(quest, "optional_objectives"))
    return (*primary, *optional)


def _load_objective(raw_value: JsonValue, default_required: bool) -> QuestObjective:
    raw = as_mapping(raw_value)
...
def _load_card_rules(raw: JsonMap) -> CardRules:
    rules = mapping_at(raw, "multi_select_rules")
    return CardRules(
        cards=tuple(_load_card(item) for item in list_at(raw, "p0_cards")),
        default_extra_cost=mapping_at(rules, "default_extra_cost"),
        combos=tuple(_load_combo(item) for item in list_at(rules, "combo_rules")),
        conflicts=tuple(_load_conflict(item) for item in list_at(rules, "conflict_rules")),
    )
...
def _load_card(raw_value: JsonValue) -> CardRule:
    ...
    return CardRule(
        ...
        tags=string_tuple(raw.get("tags", [])),
        regions=string_tuple(raw.get("regions", [])),
        result=mapping_at(raw, "result"),
        requires_item=optional_string(raw, "requires_item"),
        requires_progress=mapping_at(raw, "requires_progress"),
        requires_status=mapping_at(raw, "requires_status"),
        applies_to_storylet_tags=string_tuple(raw.get("applies_to_storylet_tags", [])),
        applies_to_quest_objectives=string_tuple(raw.get("applies_to_quest_objectives", [])),
        progress_key=str(raw.get("progress_key", "")),
        weight_modifiers=mapping_at(raw, "weight_modifiers"),
        quest_ids=string_tuple(raw.get("quest_ids", [])),
    )


def _load_combo(raw_value: JsonValue) -> ComboRule:
    raw = as_mapping(raw_value)
    cards = string_tuple(raw.get("cards", []))
    if len(cards) != 2:
        raise CardPairCountError(str(raw.get("id", "")), "Combo")
    return ComboRule(id=str(raw["id"]), cards=(cards[0], cards[1]), result=mapping_at(raw, "result"))
